...
output_path = fr"Z:\项目\航拍特写\pexels"  # 保存路径
page_start = 1  # 开始页数
page_end = 300
DATAL_ID = '90'
mysql_config = {
...
def create_table(cursor):
    ...
    try:
        ret = cursor.execute(f"""
            CREATE TABLE IF NOT EXISTS pexels_vid_two (
...
def download_video(url, save_path):
    ...
    try:
        ...
        # 查询download_state的状态（切换为新表pexels_vid_two）
        ...
        if response.status_code == 200:
            ...
            # 更新数据库状态（切换为新表pexels_vid_two）
            with conn.cursor() as cursor:
                update_sql = """
                    UPDATE pexels_vid_two 
...
def get_existing_links():
    """获取数据库中已存在的所有下载链接（切换为新表pexels_vid_two）"""
    try:
        with mysql.connector.connect(**mysql_config) as conn:
            with conn.cursor() as cursor:
                cursor.execute("SELECT download_link FROM pexels_vid_two WHERE download_state = True")
                ...
                try:
                    with mysql.connector.connect(**mysql_config) as conn:
                        with conn.cursor() as cursor:
                            create_table(cursor)
                            sql = """
                                INSERT IGNORE INTO pexels_vid_two 
